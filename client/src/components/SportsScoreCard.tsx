...
import { Activity, ChevronLeft, ChevronRight, Trophy, Target } from 'lucide-react';
...
const SportsScoreCard: React.FC = () => {
  ...
  const [matchesData, setMatchesData] = useState<Record<Sport, any[]>>({ cricket: [], football: [] });
  ...
  return (
    <div className="mb-10 w-full overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-[#0a0a0a] p-1.5 rounded-xl border border-slate-200 dark:border-oled-border">
            {(['cricket', 'football'] as Sport[]).map(sport => (
              ...
              >
                {sport === 'cricket' ? <Trophy className="w-3.5 h-3.5" /> : <Target className="w-3.5 h-3.5" />}
                {sport}
              </button>
            ))}
          </div>
      ...
      <div className="relative group">
        ...
        <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x scroll-smooth">
        <AnimatePresence mode="wait">
          ...
          {!loading && matchesData[activeTab]?.map((match, idx) => (
            <motion.div
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <span className="font-bold text-sm text-slate-800 dark:text-slate-200 leading-tight">{match.team1}</span>
                  </div>
                  <span className="font-black text-lg tracking-tight text-slate-900 dark:text-white text-right shrink-0">{match.score1}</span>
                </div>
                ...
                <div className="flex justify-between items-center gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <span className="font-bold text-sm text-slate-800 dark:text-slate-200 leading-tight">{match.team2}</span>
                  </div>
                  <span className="font-black text-lg tracking-tight text-slate-900 dark:text-white text-right shrink-0">{match.score2}</span>
                </div>
              </div>

              <div className="mt-5 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Activity className={`w-3.5 h-3.5 ${match.isLive ? 'text-violet-500 dark:text-fuchsia-400' : 'text-slate-400'}`} />
                  <p className={`text-xs font-semibold ${match.isLive ? 'text-violet-600 dark:text-fuchsia-400' : 'text-slate-500 dark:text-slate-400'}`}>
                    {match.status}
                  </p>
                </div>
                {match.link && (
