...
const SubCategoryNav: React.FC<SubCategoryNavProps> = ({ category, activeSubCategory, setActiveSubCategory }) => {
  if (!category.subcategories || category.subcategories.length === 0) return null;

  return (
    <div className="mb-10 w-full relative">
      {/* Mobile scroll shadow indicators */}
      <div className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-white dark:from-[#02010A] to-transparent z-10 pointer-events-none sm:hidden" />
      <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-white dark:from-[#02010A] to-transparent z-10 pointer-events-none sm:hidden" />
      ...
      <div className="flex overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
        <div className="inline-flex items-center gap-1 p-1.5 bg-slate-100/80 dark:bg-slate-800/40 backdrop-blur-xl rounded-full border border-slate-200/60 dark:border-slate-700/50 shadow-inner min-w-max">
          ...
          >
            {activeSubCategory === "all" && (
              <div className="absolute inset-0 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-full" />
            )}
            <span className="relative z-10 flex items-center gap-2">
              <span className={`text-[15px] transition-all duration-300 ${activeSubCategory !== "all" ? 'opacity-70 grayscale-[50%] group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-110' : 'scale-110 drop-shadow-md'}`}>
                ✨
              </span>
              All {category.title}
            </span>
          ...
          {category.subcategories.map(sub => (
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-1.5">
                <span className={`text-[15px] transition-all duration-300 ${activeSubCategory !== sub.id ? 'opacity-70 grayscale-[50%] group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-110' : 'scale-110 drop-shadow-md'}`}>
