...
const Navbar: React.FC<NavbarProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchTrending = async () => {
      try {
        const articles = await getNewsForCategory("IN", "top", 6, { trending: true });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const articles = await getNewsForCategory("IN", "all", 5, { keyword: searchQuery });
        const titles = articles.map((a: any) => a.title).filter(Boolean);
  ...
  const displaySuggestions = searchQuery.trim() ? liveSuggestions : (trendingTopics.length > 0 ? trendingTopics : SUGGESTIONS);
      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0 group">
            <img src="/logo.svg" alt="Insta-News" className="w-8 h-8 rounded-lg shadow-md group-hover:scale-105 transition-transform duration-300" />
          ...
          {/* SEARCH (Desktop Only) */}
          <div className="hidden md:block flex-1 max-w-md mx-auto relative" ref={searchContainerRef}>
              ...
              <div className="absolute top-full left-0 right-0 mt-3 bg-white/95 dark:bg-oled-surface/95 backdrop-blur-2xl border border-violet-100 dark:border-violet-500/20 rounded-2xl shadow-[0_10px_40px_-10px_rgba(139,92,246,0.15)] overflow-hidden z-50 p-2 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                {isFetchingSuggestions && searchQuery.trim() ? (
                  ...
                  <ul className="flex flex-col gap-1 max-h-64 overflow-y-auto custom-scrollbar">
                    ...
                    {searchQuery && !displaySuggestions.some(s => s.toLowerCase() === searchQuery.toLowerCase()) && (
          ...
          <LayoutGroup>
            <div className="relative flex gap-6 lg:gap-8 py-3 overflow-x-auto scrollbar-hide lg:justify-center">
              ...
              >
                ...
                <span className="transition-transform duration-300 group-hover:-translate-y-px">Discover</span>
                {location.pathname === '/' && (
                  <motion.span
              ...
              {CATEGORIES.map((cat) => {
                const isActiveRoute = location.pathname === `/category/${cat.id}`;
                return (
                  <Link
                    key={cat.id}
                    to={`/category/${cat.id}`}
                  ...
                  >
                    {React.cloneElement(CATEGORY_ICONS[cat.id] as React.ReactElement<any>, { className: "w-4 h-4 text-inherit transition-transform duration-300 group-hover:scale-110 group-hover:-translate-y-[1px]" })}
                    <span className="transition-transform duration-300 group-hover:-translate-y-px">{cat.title}</span>
                    {isActiveRoute && (
                      <motion.span
                        layoutId="navbar-underline"
                        className="absolute left-0 right-0 -bottom-px h-0.75 gradient-primary neon-glow rounded-t-md"
