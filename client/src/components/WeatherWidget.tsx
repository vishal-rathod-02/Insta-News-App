      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200">{weather.city}</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium capitalize">{weather.condition}</p>
            </div>
          </div>
        ...
        <div className="relative z-10 flex items-center justify-between px-3 py-2 bg-slate-100/50 dark:bg-black/20 rounded-xl border border-slate-200/50 dark:border-white/5">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">High</span>
            <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{weather.high}°</span>
          </div>
          <div className="w-px h-3 bg-slate-300 dark:bg-slate-700" />
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">Low</span>
            <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{weather.low}°</span>
          </div>
        </div>
      </button>
    );
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center mb-6 mt-1">
                <div className="p-3.5 rounded-3xl bg-linear-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20 dark:border-fuchsia-500/20 mb-3 shadow-inner">
                  {renderWeatherIcon(weather.condition, "w-8 h-8 sm:w-10 sm:h-10 text-violet-600 dark:text-fuchsia-400 drop-shadow-md")}
                </div>
                <h3 className="text-xl sm:text-2xl font-black tracking-tight text-slate-800 dark:text-white mb-0.5">{weather.city}</h3>
                <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 capitalize tracking-wide">{weather.condition}</p>
                ...
                <div className="flex items-center gap-6 mt-1">
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">High</span>
                    <span className="text-base sm:text-lg font-bold text-slate-700 dark:text-slate-200">{weather.high}°</span>
                  </div>
                  <div className="w-px h-8 bg-slate-200 dark:bg-white/10" />
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Low</span>
                    <span className="text-base sm:text-lg font-bold text-slate-700 dark:text-slate-200">{weather.low}°</span>
                  </div>
                </div>
              </div>
                  ...
                  {weather.daily.map((day, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between group">
                        <span className={`w-12 sm:w-14 font-bold text-xs sm:text-sm ${i === 0 ? 'text-violet-600 dark:text-fuchsia-400' : 'text-slate-600 dark:text-slate-300'}`}>
                          {dayName}
                        </span>
                        ...
                        <div className="flex-1 flex justify-center">
                          <div className="p-1 rounded-full bg-slate-200/50 dark:bg-white/5 group-hover:bg-violet-100 dark:group-hover:bg-violet-500/20 transition-colors">
                            {renderWeatherIcon(getWeatherCondition(day.conditionCode), "w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-500 dark:text-slate-400 group-hover:text-violet-600 dark:group-hover:text-fuchsia-400")}
                          </div>
                        </div>

                        <div className="flex items-center gap-2 sm:gap-3 w-20 sm:w-24 justify-end">
                          <span className="text-[10px] sm:text-xs font-semibold text-slate-400">{day.minTemp}°</span>
                          {/* Visual Temp Bar */}
                          ...
                          <span className="text-[10px] sm:text-xs font-bold text-slate-700 dark:text-slate-200">{day.maxTemp}°</span>
                        </div>
                      </div>
                    )
                  })}
