...
const getCategoryIcon = (id: string) => {
  const iconClass =
    "w-10 h-10 sm:w-14 sm:h-14 text-violet-600 dark:text-violet-400";
  switch (id) {
    case "business":
...
const CategoryHero: React.FC<CategoryHeroProps> = ({ category }) => {
  return (
    <div className="relative overflow-hidden rounded-4xl bg-linear-to-r from-violet-600 via-fuchsia-600 to-amber-500 p-[1.5px] mb-12 shadow-[0_20px_60px_-15px_rgba(139,92,246,0.3)]">
      <div className="relative bg-white/95 dark:bg-[#0A0510]/95 backdrop-blur-3xl rounded-4xl p-8 sm:p-12 overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-8 z-10 group">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-8 text-center sm:text-left w-full">
          ...
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 text-lg sm:text-xl font-medium max-w-2xl leading-relaxed mx-auto sm:mx-0">
              Explore the latest updates, breaking stories, and deep dives into{" "}
